...
vector<point> Points;
int N, Cycle_time=0, Upper_time, Lower_time, Robot_time, C2_time;
...
void set_points() {
    ...
    */
    ...
    C2_time = Points[N/2-1].time;
    Upper_time = C2_time / 2;
    Lower_time = C2_time + (Cycle_time - C2_time)/2;
    Robot_time = Cycle_time;
}
...
void testcase() {
    ...
    int time = seconds % Robot_time;

    if (time <= Upper_time) {
        find_time(time);
        find_time(C2_time - time);
    }
    else if (time <= C2_time) {
        find_time(C2_time - time);
        find_time(time);
    }
    else if (time <= Lower_time) {
        find_time(C2_time - time + Robot_time);
        find_time(time);
    }
    else if (time < Robot_time) {
        find_time(time);
        find_time(Robot_time - time + C2_time);
    }
}
